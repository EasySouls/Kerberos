...
namespace Kerberos
{
	...
	void EditorLayer::OnAttach()
	{
		...
		Kerberos::FramebufferSpecification frameBuferSpec;
		frameBuferSpec.Width = 1280;
		frameBuferSpec.Height = 720;
		m_Framebuffer = Kerberos::Framebuffer::Create(frameBuferSpec);
	...
	void EditorLayer::OnUpdate(const Kerberos::Timestep deltaTime)
	{
		KBR_PROFILE_FUNCTION();

		m_Fps = static_cast<float>(1) / deltaTime;

		Kerberos::Timer timer("EditorLayer::OnUpdate", [&](const ProfileResult profileResult) { m_ProfileResults.push_back(profileResult); });
	...
	void EditorLayer::OnEvent(Kerberos::Event& event)
	{
