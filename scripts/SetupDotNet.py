...
class DotNetConfiguration:
    ...
    vendorLibDir = "./Kerberos/vendor/dotnet/lib"

    @classmethod
    def Validate(cls):
        if not cls.CheckDotNetSDK():
            print("  .NET SDK not found. Please install .NET 8.0 SDK from https://dotnet.microsoft.com/download")
            return False

        if not cls.CheckNetHostLib():
            print("  nethost library not found in vendor directory. Attempting to copy from .NET SDK...")
            return cls.CopyNetHostLib()

        print("  nethost library already set up correctly.")
        return True
